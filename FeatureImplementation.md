...
# Task: 
The game state is good enoigh that we want to itterate upon it.With that in mind please itterate by having the original game code bellow attached to create the same game in Unity. The game should match as much as possible to the original game code in terms of gameplay, visuals, and features. If there is anything unclear please do not make assumptions, ask me. As last time there are things that don't work in the original game code when done in Unity make the changes, 
...
- A project that has a production ready structure that can be itterated upon. 
- Clean architercture with folders, files and assets. 
- All features from the original game code should be implemented in Unity.
- The project should be ready to be built for Mobile/Android.
- The resolution of the project should be for Full HD
...
- If in editor implementation is needed, use the original game code as a reference and write a plan on how to implement it in Unity step by step in the implementation_plan.md file that is created, there you will add a new section so i know wheere this featureimplementation begins. Be as thorugh as possible. We are on Unity version 6000.3.10f1
