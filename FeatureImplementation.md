## Itteration of the game

# Task: 
You now have created a good copy. With that in mind please itterate by having the original game code bellow attached to create the same game in Unity. The game should match as much as possible to the original game code in terms of gameplay, visuals, and features. If there is anything unclear please do not make assumptions, ask me. 

# Output: 
A Unity project that replicates the original game code.


# Rules:
- Do not make anything more than what is asked unless we continue. 
- Do not deviate from the original game code. 
- Do not create fallbacks if there is an error I would like to know about it. 
- We want to take a data driven approach so please refactor the structure of the files if needed. 
- We want this to be for Mobile/Android so I will swap the project to Android now. Please make the changes to refelct inside the project. 
...
import { Star, Lock, Unlock, Rocket, Shield, Gift, Map as MapIcon, ChevronRight, CheckCircle2, AlertTriangle, Crown, Wind, Play, RotateCcw, Box } from 'lucide-react';
...
const BATTLE_PASS_TIERS = [
  { level: 1, starsReq: 10, free: { type: 'ship', name: 'Ship B (Anti-Grav)', id: 'shipB' }, premium: { type: 'skin', name: 'Elite Skin' } },
  { level: 2, starsReq: 20, free: { type: 'ship', name: 'Ship C (Nebula)', id: 'shipC' }, premium: { type: 'powerup', name: 'Mega Power Bundle' } },
  { level: 3, starsReq: 30, free: { type: 'powerup', name: 'Small Powerup' }, premium: { type: 'buff', name: 'Shield Capacity 6/6', id: 'buffShield' } },
  { level: 4, starsReq: 40, free: { type: 'skin', name: 'Rookie Skin' }, premium: { type: 'trail', name: 'Neon Trail', id: 'trailNeon' } },
  { level: 5, starsReq: 50, free: { type: 'skin', name: 'Veteran Skin' }, premium: { type: 'perk', name: 'Free Replay Token / Sector' } },
  { level: 6, starsReq: 60, free: { type: 'trail', name: 'Plasma Trail', id: 'trailPlasma' }, premium: { type: 'skin', name: 'Galactic Skin' } },
  { level: 7, starsReq: 70, free: { type: 'powerup', name: 'Medium Powerup' }, premium: { type: 'powerup', name: 'Ultra Power Bundle' } },
  { level: 8, starsReq: 80, free: { type: 'skin', name: 'Ace Skin' }, premium: { type: 'trail', name: 'Stardust Trail', id: 'trailStardust' } },
  { level: 9, starsReq: 90, free: { type: 'powerup', name: 'Large Powerup' }, premium: { type: 'skin', name: 'Cosmic Skin' } },
  { level: 10, starsReq: 100, free: { type: 'trail', name: 'Basic Trail', id: 'trailBasic' }, premium: { type: 'skin', name: 'Legendary Skin' } },
  { level: 11, starsReq: 110, free: { type: 'trail', name: 'Comet Trail', id: 'trailComet' }, premium: { type: 'powerup', name: 'Infinite Power Bundle' } },
  { level: 12, starsReq: 120, free: { type: 'skin', name: 'Master Skin' }, premium: { type: 'trail', name: 'Supernova Trail', id: 'trailSupernova' } },
];
...
const TerminalRow = ({ left, right, active, onComplete, isTotal = false }) => {
  ...
  // Triggers the RGB glitch shake once the perfect 3-star text finishes typing
...
const SectorSummaryOverlay = ({ u, s, scores, onClose }) => {
  ...
  return (
    <div className="absolute inset-0 flex items-center justify-center bg-black/90 backdrop-blur-sm z-[100] font-mono tracking-widest text-left">
      <div className="text-left p-8 border border-white bg-black min-w-[340px] shadow-[0_0_20px_rgba(255,255,255,0.2)]">
         <h2 className="text-xl text-white uppercase mb-4 border-b border-white pb-2 flex items-center">
           <ChevronRight className="w-5 h-5 mr-2 animate-pulse" /> Sector {s} Report
         </h2>
         ...
         {step >= 6 && (
           <button onClick={onClose} className="rgb-notify px-6 py-3 bg-white text-black border border-white hover:bg-transparent hover:text-white transition-colors font-bold uppercase tracking-widest w-full text-xs">
             Sector Completed!
           </button>
         )}
      </div>
...
const GravityGame = ({ 
  ...
  useEffect(() => {
    ...
    // Physics depend on the activeShipId state
    ...
    const handleDown = (e) => { 
      if (localState !== 'AIMING' || isFleetOpen) return; 
      isDragging = true; 
      setHudMessage(''); 
      updateAim(e); 
    };
    ...
    const draw = () => {
        ...
        // Merge ship color with trail aberration
  ...
  return (
    <div className="absolute inset-0 bg-black flex justify-center items-center z-50">
      <div className="relative w-full h-full max-w-[500px] aspect-[9/16] bg-black border-l border-r border-slate-800">
        ...
        <div className="absolute top-4 left-4 font-mono text-white tracking-widest text-sm pointer-events-none z-10 text-left">
          ...
          <div className="text-slate-400 text-xs mt-1 uppercase">Shield: <span className={currentShields > 0 ? "text-white" : "text-red-500"}>{'█'.repeat(currentShields)}{'▒'.repeat(maxShields - currentShields)}</span></div>
          <div className={`text-[10px] mt-1 font-bold uppercase transition-colors ${SHIPS[activeShipId].color}`}>Fleet: {SHIPS[activeShipId].name}</div>
        </div>
        ...
        {gameState === 'AIMING' && (
          <div className="absolute inset-0 pointer-events-none transition-opacity">
            ...
            <button 
              ...
              className="absolute bottom-6 left-6 px-4 py-3 border border-white bg-black hover:bg-white hover:text-black text-white font-mono text-[10px] uppercase tracking-widest pointer-events-auto transition-colors flex items-center"
            >
              <Rocket className="w-4 h-4 mr-2" /> Fleet
            </button>

            {/* In-Game Fleet Selection UI */}
            {isFleetOpen && (
              <div className="absolute inset-0 bg-black/80 flex items-center justify-center z-[60] pointer-events-auto backdrop-blur-sm">
                  ...
                  <div className="flex flex-col space-y-3">
                    {shipIds.map(id => (
                      <button 
                        ...
                        className={`p-4 border transition-all flex items-center justify-between ${activeShipId === id ? 'bg-white border-white' : 'bg-black/50 border-white/30 hover:border-white'}`}
                      >
                        <div className="flex items-center">
                          <Rocket className={`w-5 h-5 mr-3 ${activeShipId === id ? 'text-black' : SHIPS[id].color}`} />
                          <span className={`text-[10px] font-bold uppercase ${activeShipId === id ? 'text-black' : 'text-white'}`}>
                            {SHIPS[id].name.split(' ')[0]}
                          </span>
                        </div>
                        {activeShipId === id && <CheckCircle2 className="w-4 h-4 text-black" />}
                      </button>
                    ))}
                  </div>
                  <button 
                    onClick={() => setIsFleetOpen(false)}
                    className="w-full mt-8 py-3 border border-white/30 text-white/50 hover:bg-white hover:text-black hover:border-white transition-colors text-[10px] uppercase font-bold tracking-widest"
                  >
                    Close
                  </button>
                </div>
        ...
        {gameState === 'GAMEOVER' && (
          <div className="absolute inset-0 flex items-center justify-center bg-black/80 backdrop-blur-sm z-20">
            <div className="text-center p-8 border border-white/20 bg-black/90 min-w-[280px]">
              <h2 className="text-xl text-white tracking-widest uppercase mb-2">Signal Lost</h2>
              <p className="text-slate-400 text-sm mb-6 font-mono tracking-widest uppercase">Trajectory compromised.</p>
              {currentShields > 0 ? (
                <button onClick={() => resetLevelRef.current()} className="px-6 py-3 border border-white text-white hover:bg-white hover:text-black transition-colors font-mono uppercase tracking-widest w-full">
                  Recalculate
                </button>
              ) : (
                <div className="space-y-3">
                  <p className="text-red-400 text-xs font-bold mb-4 uppercase">Shields Depleted</p>
                  <button onClick={() => { setCurrentShields(maxShields); resetLevelRef.current(); }} className="px-6 py-3 border border-white text-white hover:bg-white hover:text-black transition-colors font-mono uppercase tracking-widest w-full flex items-center justify-center">
                    <Play className="w-4 h-4 mr-2" /> Refill Shields (Ad)
                  </button>
                  <button onClick={onExit} className="px-6 py-3 border border-white/30 text-white/70 hover:bg-white hover:text-black hover:border-white transition-colors font-mono uppercase tracking-widest w-full">
                    Retreat to Sector
                  </button>
                </div>
              )}
        ...
        {gameState === 'SUCCESS' && (
          <div className="absolute inset-0 flex items-center justify-center bg-black/80 backdrop-blur-sm z-20">
            <div className="text-center p-8 border border-white/20 bg-black/90 min-w-[280px]">
              <h2 className="text-xl text-white tracking-widest uppercase mb-4 tracking-tighter">Docking Successful</h2>
              <div className="flex justify-center space-x-2 mb-6">
                 {[1, 2, 3].map(s => <Star key={s} className={`w-10 h-10 ${s <= starsEarned ? 'text-white fill-current' : 'text-slate-700'}`} />)}
              </div>
...
export default function App() {
  ...
  const claimReward = (tierLevel, isPremiumReward) => {
    ...
    if (!claimedRewards.includes(rewardId)) setClaimedRewards(prev => [...prev, rewardId]);
  };
  ...
  // --- RENDERERS ---
  const renderTopNav = () => (
    <div className="relative z-10 flex flex-wrap items-center justify-between p-4 bg-black border-b border-white font-mono tracking-widest">
      <div className="flex space-x-6 items-center">
        <div className="flex items-center text-white font-bold text-xl uppercase">
          <Star className="w-6 h-6 mr-2 fill-current" /> {totalStars}
        </div>
        <div className="flex items-center text-white font-bold text-xl uppercase">
          <Shield className="w-6 h-6 mr-2" /> LVL {accountLevel}
        </div>
        <div className="hidden md:flex items-center text-white text-sm border border-white px-3 py-1 uppercase space-x-3 text-left">
          <span className="flex items-center"><Rocket className="w-4 h-4 mr-1" /> FLEET ({activeShips.length})</span>
          <span className="text-white/50">|</span>
          <span className="flex items-center"><Wind className="w-4 h-4 mr-1" /> {TRAILS[activeTrail].name}</span>
        </div>
      </div>

      <div className="flex space-x-2">
        <button onClick={() => setView('map')} className={`px-4 py-2 border text-xs font-bold uppercase tracking-widest flex items-center transition-colors ${view === 'map' ? 'bg-white text-black border-white' : 'bg-black text-white border-white hover:bg-white hover:text-black'}`}><MapIcon className="w-4 h-4 mr-2" /> Map</button>
        <button onClick={() => setView('battlepass')} className={`px-4 py-2 border text-xs font-bold uppercase tracking-widest flex items-center transition-colors ${view === 'battlepass' ? 'bg-white text-black border-white' : 'bg-black text-white border-white hover:bg-white hover:text-black'} ${hasUnclaimedRewards && view !== 'battlepass' ? 'rgb-notify' : ''}`}>
          <Gift className="w-4 h-4 mr-2" /> Pass
          {hasUnclaimedRewards && view !== 'battlepass' && <span className="ml-2 w-2 h-2 bg-white rounded-full animate-pulse"></span>}
        </button>
        <button onClick={() => setView('hangar')} className={`px-4 py-2 border text-xs font-bold uppercase tracking-widest flex items-center transition-colors ${view === 'hangar' ? 'bg-white text-black border-white' : 'bg-black text-white border-white hover:bg-white hover:text-black'}`}><Rocket className="w-4 h-4 mr-2" /> Hangar</button>
      </div>
    </div>
  );
  ...
  const renderMapView = () => {
    return (
      <div className="relative z-10 p-8 max-w-5xl mx-auto h-[calc(100vh-80px)] overflow-y-auto font-mono tracking-widest text-left">
        <div className="flex justify-between items-center mb-8 border-b border-white pb-4">
          <h1 className="text-3xl font-bold text-white uppercase">UNIVERSE {currentUniverse}</h1>
          <div className="flex space-x-2">
            ...
            <button onClick={() => canEnterUniverse2 && setCurrentUniverse(2)} className={`px-4 py-2 border text-xs font-bold uppercase flex items-center transition-colors ${currentUniverse === 2 ? 'bg-white text-black border-white' : canEnterUniverse2 ? 'bg-black text-white border-white hover:bg-white hover:text-black' : 'bg-black text-white/30 border-white/30 cursor-not-allowed border-dashed'}`}>
              U2 {!canEnterUniverse2 && <Lock className="w-3 h-3 ml-2" />}
            </button>
          </div>
        </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-5 gap-6">
          {[...Array(10)].map((_, i) => {
            ...
            let badgeColor = 'border-white text-white';
            return (
              <div key={sectorNum} onClick={() => isUnlocked && setActiveSector({ u: currentUniverse, s: sectorNum, mechs: mechanics })} className={`relative group p-6 border transition-all flex flex-col ${isUnlocked ? 'cursor-pointer hover:bg-white hover:text-black text-white bg-black border-white' : 'opacity-50 cursor-not-allowed text-white/30 bg-black border-white/30 border-dashed'}`}>
                <div className="flex justify-between items-start mb-1">
                  ...
                  {!isUnlocked ? <Lock className="w-5 h-5" /> : <ChevronRight className={`w-5 h-5 opacity-0 group-hover:opacity-100 transition-opacity ${isUnlocked ? 'group-hover:text-black text-white' : ''}`} />}
                </div>
                ...
                <div className="mt-auto flex flex-col space-y-1 text-left">
                  <div className="text-xs uppercase font-bold">LVL: {levelsCleared}/5</div>
                  <div className="flex items-center text-sm font-bold">{sectorStars}/15 <Star className="w-3 h-3 ml-1 fill-current" /></div>
                </div>
              </div>
            );
          })}
  ...
  const renderSectorView = () => {
    ...
    return (
      <div className="relative z-10 p-8 max-w-4xl mx-auto h-[calc(100vh-80px)] overflow-y-auto pb-20 font-mono tracking-widest text-left">
        <button onClick={() => setActiveSector(null)} className="mb-6 text-white hover:bg-white hover:text-black border border-transparent hover:border-white px-2 py-1 transition-colors flex items-center text-xs uppercase font-bold w-fit">
          <ChevronRight className="w-4 h-4 mr-1 transform rotate-180" /> Back
        </button>
        
        <div className="bg-black border border-white p-8">
          <div className="flex justify-between items-center mb-2 border-b border-white pb-4">
            <h2 className="text-3xl font-bold text-white uppercase">Universe {u} - Sector {s}</h2>
            {isSectorBeaten && hasNextSector && (
              <button 
                onClick={() => setActiveSector({ u, s: s + 1, mechs: getSectorMechanicsList(s + 1).map(id => SECTOR_MECHANICS[id]) })}
                className="px-4 py-2 border border-white text-white hover:bg-white hover:text-black transition-colors font-bold uppercase text-xs tracking-widest flex items-center"
              >
                Next Sector <ChevronRight className="w-4 h-4 ml-1" />
              </button>
            )}
          </div>
          
          <div className="grid grid-cols-1 gap-2 mt-6 mb-8 text-left">
            {mechs.map((mech, i) => {
               ...
               return (
                  <div key={i} className={`p-4 border flex items-start ${isMet ? `bg-black ${mech.color}` : 'bg-white text-black border-black'}`}>
                    ...
                    <div>
                      <h4 className="font-bold uppercase text-sm">Hazard: {mech.name}</h4>
                      <p className="text-xs mt-1">{mech.warning}</p>
                      {!isMet && <p className="text-[10px] font-bold mt-2 uppercase text-red-600">WARNING: Correct ship not equipped in Fleet!</p>}
                    </div>
                  </div>
               )
            })}
            ...
            {[...Array(5)].map((_, i) => {
              ...
              return (
                <div key={l} className={`flex items-center justify-between p-4 border ${canPlayFromMenu ? 'bg-black border-white text-white' : 'bg-black border-white/30 border-dashed text-white/30'}`}>
                  <div className="flex items-center">
                    <div className={`w-10 h-10 flex items-center justify-center font-bold border mr-4 ${canPlayFromMenu ? 'border-white' : 'border-white/30'}`}>{l}</div>
                    <div className="text-left">
                      <h4 className="font-bold flex items-center uppercase text-sm">Level {l} {isCleared && <span className="ml-2 text-[10px] border px-1 uppercase">Cleared</span>}</h4>
                      <div className="flex mt-1 text-left">
                        {[1, 2, 3].map(starIdx => ( <Star key={starIdx} className={`w-4 h-4 mr-1 ${starIdx <= stars ? 'fill-current' : 'opacity-30'}`} /> ))}
                      </div>
                    </div>
                  </div>
                  ...
                  {canPlayFromMenu ? (
                    <button 
                      ...
                      className={`px-6 py-3 border font-bold transition-colors flex items-center text-xs uppercase tracking-widest ${requiresAd ? 'bg-black text-white border-white hover:bg-white hover:text-black' : 'bg-white text-black hover:bg-transparent hover:text-white border-white'}`}
                    >
                      <Play className="w-4 h-4 mr-2 fill-current" /> PRE-FLIGHT
                    </button>
                  ) : (
                    <div className="flex items-center text-[10px] uppercase tracking-widest text-white/50 mr-2 text-right">
                        <Lock className="w-4 h-4 mr-2" /> Reach via Lvl {l-1}
                    </div>
                  )}
                </div>
              );
  ...
  const renderLoadoutPanel = () => {
    ...
    return (
      <div className="absolute inset-0 flex items-center justify-center bg-black/90 backdrop-blur-md z-[150] font-mono tracking-widest overflow-y-auto py-10 text-left">
        <div className="text-left p-8 border border-white bg-black w-full max-w-4xl shadow-[0_0_30px_rgba(255,255,255,0.1)]">
           <div className="flex justify-between items-end border-b border-white pb-4 mb-8">
              <div className="text-left">
                <div className="text-xs text-white/50 mb-1 uppercase">Transmission Secure</div>
                <h2 className="text-2xl text-white uppercase font-bold">Pre-Flight Loadout</h2>
              </div>
              <div className="text-right">
                <div className="text-xs text-white/50 mb-1 uppercase">Target Coords</div>
                <h3 className="text-xl text-white uppercase font-bold">U{u} - S{s} - L{l}</h3>
              </div>
           </div>
                 ...
                 {Object.values(SHIPS).filter(ship => unlockedShips.includes(ship.id)).map(ship => {
                   ...
                   return (
                     <div key={ship.id} onClick={() => toggleShipEquip(ship.id)} className={`cursor-pointer border p-3 flex items-center justify-between transition-colors ${isEquipped ? 'bg-white text-black border-white' : 'bg-black text-white/50 border-white/30 hover:border-white hover:text-white'}`}>
                       <div className="flex items-center">
                         <Rocket className={`w-5 h-5 mr-3 ${isEquipped ? 'text-black' : ship.color}`} />
                         <span className="text-xs uppercase font-bold">{ship.name}</span>
                       </div>
             ...
             <div>
                ...
                <div className="space-y-3">
                  {mechs.map((mech, i) => {
                    const isMet = !mech.reqShip || activeShips.includes(mech.reqShip);
                    return (
                      <div key={i} className={`p-3 border flex flex-col ${isMet ? `bg-black border-white ${mech.color}` : 'bg-black border-red-500 text-red-500'}`}>
                        <div className="flex items-center mb-1 text-xs uppercase font-bold">
                           <AlertTriangle className="w-4 h-4 mr-2" /> {mech.name}
                        </div>
                        {!isMet && <span className="text-[10px] uppercase">Missing: {SHIPS[mech.reqShip].name}</span>}
                        {isMet && <span className="text-[10px] uppercase opacity-70">Addressed by Fleet</span>}
                      </div>
                    )
                  })}
                </div>
           ...
           <div className="flex space-x-4">
              <button 
                ...
                className={`flex-1 px-6 py-4 transition-colors font-bold uppercase tracking-widest flex items-center justify-center text-sm ${requiresAd ? 'rgb-notify bg-white text-black border border-white' : 'bg-white text-black hover:bg-transparent hover:text-white border border-white'}`}
              >
                <Play className="w-5 h-5 mr-2 fill-current" /> {requiresAd ? 'WATCH AD & LAUNCH' : 'LAUNCH MISSION'}
              </button>
              <button onClick={() => setPendingLoadout(null)} className="px-6 py-4 border border-white/30 text-white/70 hover:bg-white hover:text-black hover:border-white transition-colors font-bold uppercase tracking-widest text-xs">
                Abort
              </button>
           </div>
        </div>
  ...
  const renderBattlePass = () => (
      <div className="relative z-10 p-8 h-[calc(100vh-80px)] flex flex-col font-mono tracking-widest text-left">
        <div className="flex justify-between items-center mb-8 max-w-6xl mx-auto w-full shrink-0 border-b border-white pb-4">
          <div className="text-left">
            <h1 className="text-3xl font-bold text-white mb-2 uppercase">Progression</h1>
            <p className="text-white/70 text-xs uppercase">Earn stars to unlock. Current Level: <span className="text-white font-bold text-sm">[{accountLevel}]</span></p>
          </div>
          <div className="bg-black p-4 border border-white flex flex-col items-center text-center">
            <span className="text-[10px] font-bold text-white/70 mb-2 uppercase">Pass Status</span>
            {isPremium ? (
              <span className="px-4 py-1.5 bg-white text-black font-bold text-xs flex items-center uppercase"><Crown className="w-4 h-4 mr-2" /> Premium Active</span>
            ) : (
              <button onClick={() => setIsPremium(true)} className="rgb-notify px-4 py-1.5 border border-white bg-transparent text-white hover:bg-white hover:text-black font-bold text-xs flex items-center transition-colors uppercase"><Crown className="w-4 h-4 mr-2" /> Upgrade Pass</button>
            )}
          </div>
        </div>
        <div className="flex-1 overflow-x-auto pb-8 hide-scrollbar">
          <div className="flex space-x-4 min-w-max px-8">
            {BATTLE_PASS_TIERS.map((tier) => {
              ...
              return (
                <div key={tier.level} className={`w-64 flex flex-col shrink-0 border transition-all ${isUnlocked ? 'border-white bg-black' : 'border-white/30 border-dashed bg-black opacity-70'}`}>
                  <div className={`p-4 border-b flex justify-between items-center ${isUnlocked ? 'border-white bg-white text-black' : 'border-white/30 text-white/70'}`}>
                    <span className="font-bold text-sm uppercase">Tier {tier.level}</span>
                    <span className="text-[10px] font-bold flex items-center uppercase border border-current px-2 py-1">{tier.starsReq} <Star className="w-3 h-3 ml-1 fill-current" /></span>
                  </div>
                  <div className="p-4 flex-1 flex flex-col justify-between border-b border-white/50 relative text-white text-left">
                    <div className="text-left"><div className="text-[10px] font-bold text-white/50 mb-2 uppercase">FREE</div><div className={`font-bold text-sm uppercase ${tier.free.type === 'ship' ? 'underline text-blue-300' : ''}`}>{tier.free.type === 'ship' && <Rocket className="w-4 h-4 inline mr-1 mb-1" />} {tier.free.name}</div></div>
                    <div className="mt-4 text-left">{isUnlocked ? ( freeClaimed ? <div className="text-xs font-bold flex items-center uppercase border border-white px-2 py-1 w-fit"><CheckCircle2 className="w-3 h-3 mr-1" /> Claimed</div> : <button onClick={() => claimReward(tier.level, false)} className="rgb-notify w-full py-2 bg-transparent hover:bg-white border border-white text-white hover:text-black text-xs font-bold transition-colors uppercase tracking-widest text-center">Claim Free</button> ) : <Lock className="w-5 h-5 text-white/30 mx-auto" />}</div>
                  </div>
                  <div className={`p-4 flex-1 flex flex-col justify-between relative ${isPremium ? 'bg-white text-black' : 'bg-black text-white'} text-left`}>
                    {!isPremium && <div className="absolute inset-0 bg-black/80 flex items-center justify-center border-t border-white/30"><Lock className="w-6 h-6 text-white/50" /></div>}
                    <div className="text-left"><div className="text-[10px] font-bold mb-2 flex items-center uppercase"><Crown className="w-3 h-3 mr-1" /> PREMIUM</div><div className="font-bold text-sm uppercase">{tier.premium.name}</div></div>
                    <div className="mt-4 z-20 text-left">{isUnlocked && isPremium ? ( premiumClaimed ? <div className="text-xs font-bold flex items-center uppercase border border-black px-2 py-1 w-fit"><CheckCircle2 className="w-3 h-3 mr-1" /> Claimed</div> : <button onClick={() => claimReward(tier.level, true)} className="rgb-notify w-full py-2 bg-black hover:bg-transparent text-white hover:text-black border border-black text-xs font-bold transition-colors uppercase tracking-widest text-center">Claim Prem</button> ) : null}</div>
                  </div>
                </div>
              );
            })}
  ...
  const renderHangar = () => (
      <div className="relative z-10 p-8 max-w-5xl mx-auto h-[calc(100vh-80px)] overflow-y-auto pb-20 font-mono tracking-widest text-left">
         <h1 className="text-3xl font-bold text-white mb-8 uppercase border-b border-white pb-4 text-left">Hangar Terminal</h1>
         ...
         <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-12 text-left">
            {Object.values(SHIPS).map(ship => {
              ...
              const isEquipped = activeShips.includes(ship.id);
              return (
                <div key={ship.id} className={`border p-6 transition-all flex flex-col ${isEquipped ? 'border-white bg-white text-black shadow-[0_0_15px_rgba(255,255,255,0.3)]' : isUnlocked ? 'border-white/50 bg-black text-white hover:border-white' : 'border-white/20 bg-black text-white/30 border-dashed'}`}>
                  <div className="flex justify-between items-start mb-4">
                    <Rocket className={`w-10 h-10 ${isEquipped ? 'text-black' : ship.color}`} strokeWidth={isEquipped ? 2 : 1.5} />
                    {isEquipped && <span className="bg-black text-white text-[10px] font-bold px-2 py-1 uppercase border border-black">Active</span>}
                    {!isUnlocked && <Lock className="w-5 h-5 text-white/30" />}
                  </div>
                  <h3 className="text-lg font-bold mb-2 uppercase text-left">{ship.name}</h3>
                  <p className={`text-xs mb-6 h-10 text-left ${isEquipped ? 'text-black/70' : isUnlocked ? 'text-white/70' : 'text-white/30'}`}>{ship.perk}</p>
                  
                  <div className="mt-auto text-center">
                    {isUnlocked ? (
                      <button 
                        onClick={() => toggleShipEquip(ship.id)} 
                        className={`w-full py-3 font-bold uppercase text-xs tracking-widest transition-colors ${isEquipped ? 'border border-black bg-black text-white hover:bg-transparent hover:text-black' : 'border border-white bg-transparent text-white hover:bg-white hover:text-black'}`}
                      >
                        {isEquipped ? 'Detach' : 'Assign to Fleet'}
                      </button>
                    ) : (
                      <button disabled className="w-full py-3 border border-white/20 bg-transparent text-white/30 text-xs uppercase tracking-widest cursor-not-allowed">
                        Locked
                      </button>
                    )}
                  </div>
         ...
         <h2 className="text-xl font-bold text-white/70 mb-6 uppercase text-left">Exhaust / Trails</h2>
         <div className="grid grid-cols-1 md:grid-cols-3 gap-6 text-left">
            {Object.values(TRAILS).map(trail => {
              ...
              return (
                <div key={trail.id} className={`border p-6 transition-all flex flex-col ${isEquipped ? 'border-white bg-white text-black shadow-[0_0_15px_rgba(255,255,255,0.3)]' : isUnlocked ? 'border-white/50 bg-black text-white hover:border-white' : 'border-white/20 bg-black text-white/30 border-dashed'}`}>
                  <div className="flex justify-between items-start mb-4">
                    <Wind className={`w-10 h-10 ${isEquipped ? 'text-black' : isUnlocked ? 'text-white' : 'text-white/30'}`} strokeWidth={isEquipped ? 2 : 1.5} />
                    {isEquipped && <span className="bg-black text-white text-[10px] font-bold px-2 py-1 uppercase border border-black">Active</span>}
                    {!isUnlocked && <Lock className="w-5 h-5 text-white/30" />}
                  </div>
                  <h3 className="text-lg font-bold mb-2 uppercase text-left">{trail.name}</h3>
                  <p className={`text-xs mb-6 h-10 text-left ${isEquipped ? 'text-black/70' : isUnlocked ? 'text-white/70' : 'text-white/30'}`}>{trail.perk}</p>
                  ...
                  <div className="mt-auto text-center">
                    {isUnlocked ? (
                      <button 
                        ...
                        className={`w-full py-3 font-bold uppercase text-xs tracking-widest transition-colors ${isEquipped ? 'border border-black bg-black text-white cursor-default' : 'border border-white bg-transparent text-white hover:bg-white hover:text-black'}`}
                      >
                        {isEquipped ? 'Equipped' : 'Initialize'}
                      </button>
                    ) : (
                      <button disabled className="w-full py-3 border border-white/20 bg-transparent text-white/30 text-xs uppercase tracking-widest cursor-not-allowed">
                        Locked
    ...
    <div className="min-h-screen bg-black font-mono text-white overflow-hidden selection:bg-white selection:text-black">
      ...
      {playSession ? (
        ...
        <>
          ...
          {adOverlay && (
            <div className="absolute inset-0 bg-black z-[200] flex justify-center items-center font-mono text-white tracking-widest uppercase">
